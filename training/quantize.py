...
def quantize(build_dir, quant_mode, batchsize):
  ...
  #image datasets
  train_loader, test_loader = get_asl_dataloader(batch_size=batchsize)
  
  data_iter = iter(train_loader)
  images, labels = next(data_iter)

  # grab the first sample from the batch
  sample_img = images[0]      # shape: [C, H, W]
  sample_img = sample_img.unsqueeze(0)
  
  quantizer = torch_quantizer(quant_mode, model, (sample_img), output_dir=quant_model) 
